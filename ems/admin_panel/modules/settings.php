...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['office_start_time'])) {
        try {
            $settings = ['office_start_time', 'late_start_time', 'half_day_time', 'half_day_checkout_time', 'office_end_time', 'saturday_end_time', 'attendance_window_start', 'attendance_window_end'];
    ...
    // Face verification policy. Super admin only, because turning it off or
    // loosening the threshold is what the feature exists to prevent.
    if (isset($_POST['face_verification_mode'])) {
        if (getAdminRoleName() !== 'super_admin') {
            ...
            try {
                ...
                // Clamped to the same range the server enforces, so the saved
                // value can never be one the matcher will quietly ignore.
    ...
    if (isset($_POST['rule_name'])) {
        try {
            // overtime_period_minutes is not written: there is no such rule and
            // no field for it, so every save was silently resetting it to 60.
            // The late columns are no longer written: lateness is not charged, the
            // fields are gone from the form, and saving would otherwise reset
            // them to 0 from a $_POST value that is never sent.
...
<div class="row g-3">
    ...
    <!-- The face-check log panel was removed from this page on request.
         Verifications are still recorded in face_verification_log, so the
         scores remain available if the threshold ever needs reviewing. -->
</div>
